...
### `Client`
Stateless — holds `serverEntryAddr` and `token`. Created via `NewClient(addr, token)`.
...
Dials the entry listener TCP, optionally sends a token line, waits for "OK\n" response. Uses a 10 s dial timeout and 5 s handshake read deadline.
...
* Token handshake uses line-based protocol: send `"token\n"`, read `"OK\n"`.
