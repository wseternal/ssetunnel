...
- **Display-unavailable handling**: When `isDisplayUnavailable()` returns true (monitor off/sleeping, ScreenCaptureKit not ready after VNC disconnect), `captureAndSend` returns `transient=true` and resets `consecutiveFails`. Transient failures are retried indefinitely with exponential backoff (capped at `transientBackoffCap`) — they always resolve when the display becomes available again. There is no circuit breaker for transient failures. On macOS (CGO) this queries CoreGraphics `CGDisplayIsActive`; on other platforms (or darwin with `-tags purego`) it matches the robotgo error string (`robotgoCaptureErrSubstr`).
